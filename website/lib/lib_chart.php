...
class Chart {
   ...
   function initializeColours(){
      ...
      $this->colour[U32] = ImageColorAllocate($this->im,255,188,21);
   }
   ...
   function title_($label,$textsize,$charwidth){
      ...
      ImageFilledRectangle($this->im, $this->xo, 0, $this->w -1, MARGIN-2, $this->colour[WHITE]);
...
class BoxChart extends Chart {
   ...
   function backgroundText($a){
      ...
      foreach($a as $s){
         ...
         ImageStringUp($this->im, 3, $x, $this->h - 12, $label, $this->colour[DARK_GRAY]);
         $x = $x + $this->boxwidth + $this->boxspace;
         if ($count == $this->maxboxes){ break; } else { $count++; }
      }
   }
