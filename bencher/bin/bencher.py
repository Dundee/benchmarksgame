...
dirs = {'make': None, 'data': None}

filters = {'onlydirs': frozenset(), 'allow': [], 'only': [], 'ignore': []}
...
def checkDirectories():
    ...
    if dirs['dat_sweep'] and not isdir(dirs['dat_sweep']):
        if logger:
            logger.warn('no directory %s', dirs['dat_sweep'])

    if dirs['log_sweep'] and not isdir(dirs['log_sweep']):
        if logger:
            logger.warn('no directory %s', dirs['log_sweep'])

    if dirs['code_sweep'] and not isdir(dirs['code_sweep']):
        if logger:
            logger.warn('no directory %s', dirs['code_sweep'])
    ...
    def check(cmd):
        try:
            ...
            if e == ENOENT:  # No such file or directory
                if logger:
                    logger.debug('%s program not found', cmd[0])
...
def clean(d, srcSet):
    def cleanLocal(name, localDirName, ext, sweepDir=None, isSymlink=False):
        ...
        if obsolete and logger:
            logger.debug('cleaning %s', join(d, localDirName))
        for f in obsolete:
            os.remove(join(localdir, f))

        if sweepDir:
            sweepNames = frozenset([f for f in os.listdir(sweepDir) if f.endswith(ext)])
            obsolete = sweepNames - names
            if obsolete and logger:
                logger.debug('cleaning %s directory', sweepDir)
...
def worklist():
    ...
    w = []
    subdirs = [each for each in os.listdir(dirs['src']) if each in filters['onlydirs']]
    ...
    for d in subdirs:
        ...
        for f in programs:
            linkToSource(directory=srcdir, srcFilename=f.filename, dstDir=codedir, filename=f.programName)

            linkToSource(directory=srcdir, srcFilename=f.filename, dstDir=codedir, filename=f.codeName)
...
def callMake(p):
    ...
    with open(makeName(), 'a') as mf:
        if isexe(makeExeName):
            ...
            if logger:
                logger.debug('%s %s - %s program not found', makeExeName, p.runName, makeExeName)
                ...
                try:
                    ...
                    # compare _OUT not outFile so short name will be shown in logf
                        ...
                        if isexe(ndiffExeName):
                            ...
                            call(
                                [ndiffExeName, '-quiet', optionkv[0], optionkv[1], _OUT, argFile],
                                stdout=df,
                                stderr=STDOUT,
                            )
        ...
        with open(outName(i), 'w') as of:
            ...
            if testdata.get(testname, None):
                ...
                with open(dfName, 'r') as df:
                    m = measure(
                        a, qsplit(cmd), delay, maxtime, of, STDOUT, df, logger=logger, affinitymask=affinitymask
                    )

            # pass the test value as a command line argument in cmd
            else:
                m = measure(a, qsplit(cmd), delay, maxtime, of, STDOUT, logger=logger, affinitymask=affinitymask)
                ...
                with open(ofName, 'w') as of:
                    ...
                    with open(errName(), 'w+') as ef:
                        ...
                        if testdata.get(testname, None):
                            ...
                            with open(dfName, 'r') as df:
                                m = measure(
                                    a, qsplit(cmd), delay, maxtime, of, ef, df, logger=logger, affinitymask=affinitymask
                                )
                        ...
                        # append command line showing test value argument to log
                            ...
                            print('\nCOMMAND LINE:\n', cmd, file=logf)
                            m = measure(
                                a, qsplit(cmd), delay, maxtime, of, ef, logger=logger, affinitymask=affinitymask
                            )
...
def measurePrograms(name, programs, allowed, total):
    ...
    for p in programs:
        ...
        if logger:
            logger.info(loggerLine.getvalue())
        loggerLine.close()
...
def walkDatFiles(df, ndf, bdf):
    subdirs = [each for each in os.listdir(dirs['tmp']) if each in filters['onlydirs']]
    subdirs.sort()
    for d in subdirs:
        try:
            ...
            if logger:
                logger.info('mkcsv building csv files from %s', path)
            for f in datfiles:
                appendToCsv(d, path, f, df, ndf, bdf)
...
def makeSummaryFiles(iniName):
    ...
    try:
        if dirs['dat_sweep']:
            ...
            if logger:
                logger.info('copy %s files to %s', '*.csv', dirs['dat_sweep'])
...
def sweepLogAndCodeFiles():
    def sweep(sweepName, pattern, subdirs, dirName):
        if dirs[sweepName]:
            if logger:
                logger.info('copy %s files to %s', pattern, dirs[sweepName])
            for d in subdirs:
                try:
                    path = join(d, dirName)
...
def benchmarksGame(ini):
    ...
    if total == 0:
        ...
        if logger:
            logger.info('nothing to be done - measurements are up-to-date')
...
def force(items):
    ...
    for each in dirsToForce:
        ...
        if not isdir(d):
            if logger:
                logger.info('no %s directory - no *.dat to remove', d)
        else:
            if logger:
                logger.info('remove %s *.dat', d)
            ...
            for f in fs:
                try:
                    ...
                    if logger:
                        logger.error('%s %s %s %s', e, err, d, f)
    ...
    try:
        ...
        if ini:
            ...
            if dirs['tmp'] and isdir(dirs['tmp']):
                if not logger:
                    configureLogger(logfilemax, dirs['tmp'])
            else:
